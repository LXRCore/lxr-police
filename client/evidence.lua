...
RegisterNetEvent("evidence:client:SetStatus", function(statusId, time)
    if time > 0 and StatusList[statusId] then
        if (CurrentStatusList == nil or CurrentStatusList[statusId] == nil) or
            ...
            exports["lxr-core"]:Notify(9, CurrentStatusList[statusId].text, 5000, 0, "mp_lobby_textures", "cross", "COLOR_WHITE")
        end
...
RegisterNetEvent("evidence:client:AddEvidence", function(data)
    assert(data, "[Client] [lxr-PoliceJob-evidence:client:AddEvidence] Missing data")
    assert(data.coords, "[Client] [lxr-PoliceJob-evidence:client:AddEvidence] Missing coords")
    local categoryId = data.categoryId 
    assert(categoryId, "[Client] [lxr-PoliceJob-evidence:client:AddEvidence] Missing categoryId")
    local id = data.id
    assert(id, "[Client] [lxr-PoliceJob-evidence:client:AddEvidence] Missing id")
...
RegisterNetEvent("evidence:client:ClearBlooddropsInArea", function()
    ...
    local blooddropList = {}
    exports["lxr-core"]:Progressbar("clear_blooddrops", Lang:t("progressbar.blood_clear"), 5000, false, true, {
        ...
        if Evidences.Blooddrops and next(Evidences.Blooddrops) then
            ...
            exports["lxr-core"]:Notify(9, Lang:t("success.blood_clear"), 5000, 0, "hud_textures", "check", "COLOR_WHITE")
        end
    end, function() -- Cancel
        exports["lxr-core"]:Notify(9, Lang:t("error.blood_not_cleared"), 5000, 0, "mp_lobby_textures", "cross", "COLOR_WHITE")
    end)
end)
...
RegisterNetEvent("evidence:client:ClearCasingsInArea", function()
    local pos = GetEntityCoords(PlayerPedId())
    local casingList = {}
    exports["lxr-core"]:Progressbar("clear_casings", Lang:t("progressbar.bullet_casing"), 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true
        ...
        if Evidences.Casings and next(Evidences.Casings) then
            ...
            exports["lxr-core"]:Notify(9, Lang:t("success.bullet_casing_removed"), 5000, 0, "hud_textures", "check", "COLOR_WHITE")

        end
    end, function() -- Cancel
        exports["lxr-core"]:Notify(9, Lang:t("error.bullet_casing_not_removed"), 5000, 0, "mp_lobby_textures", "cross", "COLOR_WHITE")
    end)
end)
...
RegisterNetEvent("LXRCore:Client:OnPlayerUnloaded", function()
    shouldRunStatus = false
end)

RegisterNetEvent('LXRCore:Player:SetPlayerData', function(playerData)
